...
async function seed() {
  ...
  const certVals = [
    ...
    // Roberto Mancini - Carpentiere (sospeso)
    ...
    // Elena Ricci - Magazziniere (nuova, senza formazione)
    // Davide Moretti - Operaio Generico
    ...
    // Francesca Bruno - Operaio Edile (nuova)
    ...
    // Laura Lombardi - Responsabile HSE
  ...
  // ─── 14. Microclimate Sites ─────────────────────────────────
  const microclimateSitesVals = [
    { companyId: allCompanies[0].id, siteId: allSites[0].id, name: "Cantiere Milano - Microclima", code: "MIL-CLIMA-01", description: "Stazione microclimatica per cantiere Milano", latitude: "45.4642", longitude: "9.1900" },
    { companyId: allCompanies[1].id, siteId: allSites[2].id, name: "Sede Bologna - Microclima", code: "BO-CLIMA-01", description: "Sensore microclima sede Bologna", latitude: "44.4949", longitude: "11.3426" },
  ];
  await db.insert(microclimateSites).values(microclimateSitesVals);
  const allMicroclimateSites = await db.select().from(microclimateSites);
  console.log(`Inserted ${allMicroclimateSites.length} microclimate sites`);

  // ─── 15. Microclimate Readings ───────────────────────────────
  const microclimateReadingVals = [
    { microclimateSiteId: allMicroclimateSites[0].id, temperature: 24, humidity: 55, windSpeed: 12, pressure: 1012, co2: 420, recordedAt: new Date("2026-06-15T10:00:00Z") },
    { microclimateSiteId: allMicroclimateSites[1].id, temperature: 22, humidity: 58, windSpeed: 8, pressure: 1015, co2: 410, recordedAt: new Date("2026-06-15T10:05:00Z") },
  ];
  await db.insert(microclimateReadings).values(microclimateReadingVals);
  const allMicroclimateReadings = await db.select().from(microclimateReadings);
  console.log(`Inserted ${allMicroclimateReadings.length} microclimate readings`);

  // ─── 16. Microclimate Alerts ────────────────────────────────
  const microclimateAlertVals = [
    { microclimateSiteId: allMicroclimateSites[0].id, microclimateReadingId: allMicroclimateReadings[0].id, alertType: "temperature_high" as const, severity: "media" as const, description: "Temperatura microclima oltre soglia normale" },
    { microclimateSiteId: allMicroclimateSites[1].id, microclimateReadingId: allMicroclimateReadings[1].id, alertType: "humidity_high" as const, severity: "bassa" as const, description: "Umidità microclima leggermente elevata" },
  ];
  await db.insert(microclimateAlerts).values(microclimateAlertVals);
  const allMicroclimateAlerts = await db.select().from(microclimateAlerts);
  console.log(`Inserted ${allMicroclimateAlerts.length} microclimate alerts`);

  // ─── 17. Notification Logs ────────────────────────────────
  const notificationLogVals = [
    { userId: allWorkers[0].id, microclimateAlertId: allMicroclimateAlerts[0].id, channel: "internal" as const, status: "sent" as const, payload: "{\"message\": \"Temperatura alta\"}", response: "OK", sentAt: new Date("2026-06-15T10:01:00Z") },
  ];
  await db.insert(notificationLogs).values(notificationLogVals);
  const allNotificationLogs = await db.select().from(notificationLogs);
  console.log(`Inserted ${allNotificationLogs.length} notification logs`);

  // ─── 18. OT23 Compliance ──────────────────────────────────
  const ot23Vals = [
    { companyId: allCompanies[0].id, siteId: allSites[0].id, documentId: allDocuments?.[0]?.id ?? null, year: 2026, status: "compliant" as const, assessedAt: "2026-06-01", notes: "Verifica OT23 eseguita con esito positivo" },
    { companyId: allCompanies[1].id, siteId: allSites[2].id, documentId: allDocuments?.[1]?.id ?? null, year: 2026, status: "review_required" as const, assessedAt: "2026-06-01", notes: "Documentazione OT23 in revisione" },
  ];
  await db.insert(ot23Compliance).values(ot23Vals);
  const allOt23Compliance = await db.select().from(ot23Compliance);
  console.log(`Inserted ${allOt23Compliance.length} OT23 compliance records`);
